# ruby leetcode/easy/788-rotated-digits.rb
...
def rotated_digits(n)
  ...
  non_rotatable_digits = [3, 4, 7]
  rotated_automatic_digits = [0, 1, 8]
  
  (1..n).each do |num|
    digits = num.digits
    
    next if digits.any? { |digit| non_rotatable_digits.include?(digit) }
    next if digits.all? { |digit| rotated_automatic_digits.include?(digit) }
