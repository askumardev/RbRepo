# ruby leetcode/easy/819-most-common-word.rb
...
def most_common_word(paragraph, banned)
  ...
  paragraph.reject! { |word| banned.include?(word.downcase) }
      
  max_word = find_max_word(paragraph.join(' '))
end

def find_max_word(paragraph)
  ...
  paragraph.split(' ').each do |word|
    ...
    if words_hash[word] > max_occurrences
      max_word = word
      max_occurrences = words_hash[word]
    end
  end
